  ...
  <div class="controls">
    ...
    <div class="chant-selector">
      {#each chants as chant, index}
        ...
        >
          <div class="chant-text">{String.fromCharCode(65 + index)}</div>
        </button>
      {/each}
    </div>
    {#if !isPlaying}
      <button onclick={handleStart} class="btn btn-start"
        ><span class="material-symbols-rounded">play_arrow</span></button
      >
    {:else}
      <button
        onclick={handlePause}
        class="btn btn-pause"
        class:startAgain={isPaused}
      >
        <span class="material-symbols-rounded"
          >{isPaused ? "play_arrow" : "pause"}</span
        >
      </button>
    {/if}
...
<style>
  ...
  .chant-btn.active .chant-text {
    background: rgba(61, 57, 57, 0.66);
    box-shadow:
      0px 2px 1px rgba(0, 0, 0, 0.63),
      inset 0px 2px 1px rgba(0, 0, 0, 0.37);
